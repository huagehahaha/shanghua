<?php
/**
 * [ShangHua System] Copyright (c) 2017 AAYKK
 * $sn$
 */
...
function mc_parse_profile($profile) {
	...
	//邮箱无效不显示。
	if(empty($profile['email']) || (!empty($profile['email']) && substr($profile['email'], -6) == 'aaykk.com' && strlen($profile['email']) == 39)) {
		$profile['email'] = '';
	}
	return $profile;
};
		...
		if (!empty($list)) {
			...
			for ($i = 0; $i < $size; $i++) {
				...
				foreach ($buffer as $row) {
					if (strexists($row['email'], 'aaykk.com')) {
						$row['email'] = '';
					}
